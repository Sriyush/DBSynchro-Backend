...
async function readGoogleSheet(token, sheetId, sheetName) {
...
async function appendRowToSheet(accessToken, sheetId, range, values) {
    try {
        const auth = new googleapis_1.google.auth.OAuth2();
        auth.setCredentials({ access_token: accessToken });
        const sheets = googleapis_1.google.sheets({ version: "v4", auth });
...
async function addColumnToSheet(accessToken, sheetId, sheetTab, columnName) {
...
async function updateSheetRow(accessToken, sheetId, sheetTab, searchRow, // key=header, value=current_db_value (before update)
newRowData // key=header, value=new_value
) {
    try {
        const auth = new googleapis_1.google.auth.OAuth2();
        auth.setCredentials({ access_token: accessToken });
        ...
        for (let i = 0; i < dataRows.length; i++) {
            ...
            for (const [header, val] of Object.entries(searchRow)) {
                ...
                if (colIndex === -1)
                    continue; // Header not in sheet? Skip check.
